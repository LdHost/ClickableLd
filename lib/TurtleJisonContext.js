...
class TurtleJisonContext {
  ...
  makeFirstRest (startToken, elts, ws1, endToken) {
    console.log("makeFirstRest:", startToken, JSON.stringify(elts, null, 2), ws1, endToken)
    const ret = {
      type: "collection",
      startToken,
      elts,
  ...
  startBlankNodePropertyList (startToken, startElts) {
    ...
    console.log('startBlankNodePropertyList:', startToken, startElts);
    ...
    return {startToken, startElts, subject, predicate}
  }

  finishBlankNodePropertyList ({startToken, startElts, subject, predicate}, midElts, endToken) {
    endToken.ord = startToken.ord;
    console.log('finishBlankNodePropertyList:', startToken, startElts, subject, predicate, midElts, endToken, "</>")
    this.setSubject(subject);
    this.setPredicate(predicate);
    return {node: startToken, elts: [startToken].concat(startElts, midElts, [endToken])};
  }
  ...
  finishTriple (o, elts) {console.log('finishTriple:', this.curSubject, this.curPredicate, o.term)
